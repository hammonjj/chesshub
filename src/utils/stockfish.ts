...
/* Stockfish Parsing */
export function parseStockfishOutput(output: string, turn: Pieces): StockfishAnalysisResult | null {
  ...
  // const moveUCI = output.match(/(?: pv )(.+?)(?= |$)/)?.[1];
  ...
  return {
    ...
    score: (parseInt(scoreMatch[1], 10) / 100) * (turn === "White" ? 1 : -1),
...
export function parsePolyglotEntry(buffer: ArrayBufferLike & { BYTES_PER_ELEMENT?: undefined }, entryIndex: number) {
